...
function renderSummary(){const front=totals("front"),packer=totals("packer"),frontBudget=state.week.cashierBudgetMinutes,packerBudget=state.week.packerBudgetMinutes;const byRole=role=>state.shifts.filter(s=>s.role===role).reduce((sum,s)=>sum+s.paidMinutes,0);const cashier=byRole("cashier"),supervisor=byRole("supervisor");$("#combinedPlanned").textContent=durationText(cashier+supervisor+packer);$("#combinedBreakdown").textContent=`Caissières ${durationText(cashier)} + superviseurs ${durationText(supervisor)} + emballeurs ${durationText(packer)}`;$("#frontBudget").value=frontBudget/60;$("#packerBudget").value=packerBudget/60;$("#frontPlanned").textContent=durationText(front);$("#packerPlanned").textContent=durationText(packer);for(const [id,actual,budget] of [["frontVariance",front,frontBudget],["packerVariance",packer,packerBudget]]){const node=$("#"+id);node.textContent=varianceText(actual,budget);node.className=actual===budget?"good":"bad";}}
...
function renderPrint(){
  ...
  const buildSection=group=>{
    ...
    const employees=state.employees.filter(e=>(group.roles.includes(e.role)||groupShifts.some(shift=>state.assignments.some(a=>a.shiftId===shift.id&&a.employeeId===e.id)))&&(e.active||state.assignments.some(a=>a.employeeId===e.id))).sort((a,b)=>(a.displayRank??9999)-(b.displayRank??9999)||a.seniority.localeCompare(b.seniority)||a.id-b.id);
...
function shiftAvailabilityPanel(shift){
  const people=state.employees.filter(e=>canWorkRole(e,shift.role)&&e.active).sort((a,b)=>(a.displayRank??9999)-(b.displayRank??9999));
...
function renderSchedule(){
  ...
  $("#scheduleBoard").innerHTML=scheduleGroups.map(group=>`<section class="schedule-group"><h3>${group.title}</h3><div class="schedule-grid">${dayNames.map((name,day)=>`<div class="schedule-day"><h4>${name}</h4>${state.shifts.filter(s=>group.roles.includes(s.role)&&s.dayIndex===day).sort((a,b)=>a.startMinute-b.startMinute).map(s=>`<label class="schedule-item${s.role!=="support"&&!state.assignments.some(a=>a.shiftId===s.id)?" schedule-item-unfilled":""}"><span>${timeText(s.startMinute)}–${timeText(s.endMinute)}${s.role==="support"?` · ${roleNames.support}`:""}</span><select data-shift="${s.id}" ${s.role==="support"?"disabled":""}><option value="">${s.role==="support"?"Autre département":"À couvrir"}</option>${state.employees.filter(e=>canWorkRole(e,s.role)&&(availableForShift(e,s)||state.assignments.some(a=>a.shiftId===s.id&&a.employeeId===e.id))).map(e=>`<option value="${e.id}" ${state.assignments.some(a=>a.shiftId===s.id&&a.employeeId===e.id)?"selected":""}>${escapeHtml(e.name)}${availableForShift(e,s)?"":" (déjà attribué, hors disponibilité)"}</option>`).join("")}</select></label>${s.role!=="support"&&!state.assignments.some(a=>a.shiftId===s.id)?shiftAvailabilityPanel(s):""}`).join("")||"<small>Aucun quart</small>"}</div>`).join("")}</div></section>`).join("");
  $("#employeeTotals").innerHTML="<h3>Heures par employé</h3>"+scheduleGroups.map(group=>`<div class="total-group"><strong>${group.title}</strong>${state.employees.filter(e=>group.roles.some(role=>canWorkRole(e,role))&&state.assignments.some(a=>a.employeeId===e.id&&group.roles.includes(state.shifts.find(s=>s.id===a.shiftId)?.role))).map(e=>{const paid=state.assignments.filter(a=>a.employeeId===e.id).reduce((sum,a)=>{const shift=state.shifts.find(s=>s.id===a.shiftId);return sum+(shift&&group.roles.includes(shift.role)?shift.paidMinutes:0);},0);return `<span>${escapeHtml(e.name)} : ${durationText(paid)} / ${durationText(e.targetMinutes)} souhaitées</span>`;}).join("")||"<span>Aucun quart attribué</span>"}</div>`).join("");
